...
const SearchAi = () => {
  ...
  const handleKeyDown = async (event) => {
    ...
    if (event.key === "Enter") {
      ...
      setLoading(false)
      try {
        const res = await axios.post(`https://college-project-ecpj.onrender.com/api/chat/search`,
          { prompt },
  ...
  const FeathAllAiSearch = async () => {
    try {
      const res = await axios.get(`https://college-project-ecpj.onrender.com/api/chat/allsearch`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const DeleteHandler = async (qId) => {
    try {
      const res = await axios.delete(`https://college-project-ecpj.onrender.com/api/chat/${user._id}/query/${qId}`)
